...
export default function Index({ auth, composition, queryParams = {} }) {
                    ...
                    <div className="bg-white dark:bg-indigo-900 overflow-hidden shadow-sm sm:rounded-lg">
                        <div className="p-5 text-gray-900 dark:text-gray-100 dark:bg-indigo-950">
                            <div className="overflow-auto">
                                <h1 className="uppercase text-center font-bold text-2xl pb-2">Data Composition</h1>
                                <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-200 dark:bg-indigo-900">
                                    ...
                                    <tbody>
                                        {composition.data.length > 0 ? (
                                            composition.data.map((composition, index) => (
                                                <tr key={`${composition.id}-${index}`} className="bg-white border-b dark:bg-indigo-950 dark:border-gray-500">
                                                    <td className="px-3 py-3">{composition.id}</td>
                                                    <td className="px-3 py-3">{composition.subject}</td>
                                                    <td className="px-3 py-3">{composition.refId}</td>
                                                    <td className="px-3 py-3">{composition.nopen}</td>
                                                    <td className="px-3 py-3">{composition.sendDate}</td>
                                                </tr>
                                            ))
                                        ) : (
                                ...
                                <Pagination links={composition.links} />
                            </div>
                        </div>
                    </div>
                </div>
