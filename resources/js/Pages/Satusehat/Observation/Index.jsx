...
export default function Index({ auth, dataTable, header, totalCount, text, queryParams = {} }) {
    ...
    const searchFieldChanged = (search, value) => {
        ...
        router.get(route('observation.index'), updatedParams, {
            preserveState: true,
            preserveScroll: true,
        });
    };
                    ...
                    <div className="bg-white dark:bg-indigo-900 overflow-hidden shadow-sm sm:rounded-lg">
                        <div className="p-5 text-gray-900 dark:text-gray-100 dark:bg-indigo-950">
                            <div className="overflow-auto w-full">
                                <h1 className="uppercase text-center font-bold text-2xl pb-2">Data Observation {header} {totalCount} {text}</h1>
                                        ...
                                        <tr>
                                            <th colSpan={8} className="px-3 py-2">
                                                <div className="flex items-center space-x-2">
                                                    ...
                                                    />
                                                    <ButtonTime href={route("observation.filterByTime", "hariIni")} text="Hari Ini" />
                                                    <ButtonTime href={route("observation.filterByTime", "mingguIni")} text="Minggu Ini" />
                                                    <ButtonTime href={route("observation.filterByTime", "bulanIni")} text="Bulan Ini" />
                                                    <ButtonTime href={route("observation.filterByTime", "tahunIni")} text="Tahun Ini" />
                                                </div>
                                            </th>
                                        </tr>
                                    </thead>
                                            ...
                                            dataTable.data.map((data, index) => (
                                                <tr key={`${data.nopen}-${index}`}
                                                    ...
                                                    <td className="px-1 py-1 text-center flex items-center justify-center space-x-1">
                                                        <ButtonDetail
                                                            href={route("observation.detail", { id: data.nopen })}
                                                        />
                                                    </td>
                                                </tr>
                                            ))
