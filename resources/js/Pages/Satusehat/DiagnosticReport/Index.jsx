...
export default function Index({ auth, dataTable, header, totalCount, text, queryParams = {} }) {
    ...
    const searchFieldChanged = (search, value) => {
        ...
        router.get(route('diagnosticReport.index'), updatedParams, {
            preserveState: true,
            preserveScroll: true,
        });
    };
                    ...
                    <div className="bg-white dark:bg-indigo-900 overflow-hidden shadow-sm sm:rounded-lg">
                        <div className="p-5 text-gray-900 dark:text-gray-100 dark:bg-indigo-950">
                            <div className="overflow-auto w-full">
                                <h1 className="uppercase text-center font-bold text-2xl pb-2">Data Diagnostic Report {header} {totalCount} {text}</h1>
                                <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-200 dark:bg-indigo-900 border border-gray-500 dark:border-gray-600">
                                            ...
                                            <th colSpan={8} className="px-3 py-2">
                                                <div className="flex items-center space-x-2">
                                                    <TextInput
                                                        ...
                                                        placeholder="Cari data berdasarkan subject, ref id dan nopen"
                                                        onChange={e => onInputChange('search', e)}
                                                        onKeyPress={e => onKeyPress('search', e)}
                                                    />
                                                    <ButtonTime href={route("diagnosticReport.filterByTime", "hariIni")} text="Hari Ini" />
                                                    <ButtonTime href={route("diagnosticReport.filterByTime", "mingguIni")} text="Minggu Ini" />
                                                    <ButtonTime href={route("diagnosticReport.filterByTime", "bulanIni")} text="Bulan Ini" />
                                                    <ButtonTime href={route("diagnosticReport.filterByTime", "tahunIni")} text="Tahun Ini" />
                                                </div>
                                            </th>
                                        </tr>
                                    </thead>
                                    <thead className="text-sm font-bold text-gray-700 uppercase bg-gray-50 dark:bg-indigo-900 dark:text-yellow-500">
                                        <tr>
                                            ...
                                            <th className="px-3 py-2 border border-gray-500 dark:border-gray-600">MENU</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {dataTable.data.length > 0 ? (
                                            dataTable.data.map((data, index) => (
                                                <tr key={`${data.refId}-${index}`}
                                                    ...
                                                    <td className="px-3 py-3 border border-gray-500 dark:border-gray-600">{data.nopen}</td>
                                                    <td className="px-3 py-3 border border-gray-500 dark:border-gray-600">{data.sendDate}</td>
                                                    <td className="px-1 py-1 text-center flex items-center justify-center space-x-1">
                                                        <ButtonDetail
                                                            href={route("diagnosticReport.detail", { id: data.refId })}
                                                        />
                                                    </td>
                                                </tr>
                                            ))
