...
export default function MonitoringKegiatan({
    ...
    const headers = [
        { name: "RUANGAN ASAL" },
        { name: "RUANGAN TUJUAN" },
        { name: "TANGGAL", className: "text-wrap w-[8%]" },
        { name: "NOPEN", className: "w-[6%]" },
        { name: "NORM", className: "w-[6%]" },
        { name: "NAMA PASIEN" },
        { name: "STATUS LAYANAN", className: "text-wrap" },
    ];
    ...
    const searchFieldChanged = (search, value) => {
        ...
        router.get(route('monitoringKegiatan.index'), updatedParams, {
            preserveState: true,
            preserveScroll: true,
        });
    };
                                            ...
                                            <th colSpan={8} className="px-3 py-2">
                                                <div className="flex items-center space-x-2">
                                                    <TextInput
                                                        ...
                                                        placeholder="Cari data berdasarkan ruangan asal, ruangan tujuan, NORM, nama pasien"
                                        ...
                                        {dataTable.data.length > 0 ? (
                                            dataTable.data.map((data, index) => (
                                                <TableRow key={data.NOPEN} isEven={index % 2 === 0}>
                                                    <TableCell>{data.RUASAL}</TableCell>
                                                    <TableCell>{data.RUTUJUAN}</TableCell>
                                                    <TableCell>{data.TANGGAL}</TableCell>
                                                    <TableCell>{data.NOPEN}</TableCell>
                                                    <TableCell>{data.NORM}</TableCell>
                                                    <TableCell>{data.NAMA}</TableCell>
                                                    <TableCell>{data.STATUSLAYANAN}</TableCell>
                                                </TableRow>
                                            ))
                                        ) : (
