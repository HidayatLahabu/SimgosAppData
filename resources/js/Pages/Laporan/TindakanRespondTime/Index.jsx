...
export default function TindakanRespondTime({
    auth,
    dataTable,
    tglAwal,
    tglAkhir,
    ...
    const headers = [
        ...
        { name: "ORDER", className: "text-center w-[7%]" },
        { name: "TERIMA", className: "text-center w-[7%]" },
        { name: "HASIL", className: "text-center w-[7%]" },
        { name: "SELISIH1", className: "text-center w-[5%]" },
        { name: "SELISIH2", className: "text-center w-[5%]" },
        { name: "SELISIH3", className: "text-center w-[5%]" },
    ];
    ...
    const searchFieldChanged = (search, value) => {
        ...
        router.get(route('tindakanRespondTime.index'), updatedParams, {
            preserveState: true,
            preserveScroll: true,
        });
    };
                                            ...
                                            <th colSpan={10} className="px-3 py-2">
                                                <div className="flex items-center space-x-2">
                                                    <TextInput
                                                        ...
                                                        placeholder="Cari data berdasarkan NORM, nama pasien, ruangan atau tindakan"
                                        ...
                                        {dataTable.data.length > 0 ? (
                                            dataTable.data.map((data, index) => (
                                                <TableRow key={data.NORM} isEven={index % 2 === 0}>
                                                    ...
                                                    <TableCell className='text-center'>{data.TGL_ORDER}</TableCell>
                                                    <TableCell className='text-center'>{data.TGL_TERIMA}</TableCell>
                                                    <TableCell className='text-center'>{data.TGL_HASIL}</TableCell>
                                                    <TableCell className='text-center'>{data.SELISIH1}</TableCell>
                                                    <TableCell className='text-center'>{data.SELISIH2}</TableCell>
                                                    <TableCell className='text-center'>{data.SELISIH3}</TableCell>
                                                </TableRow>
                                            ))
                                        ) : (
