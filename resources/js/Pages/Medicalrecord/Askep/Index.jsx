...
export default function Index({ auth, dataTable, header, totalCount, queryParams = {} }) {
    ...
    const searchFieldChanged = (search, value) => {
        ...
        router.get(route('askep.index'), updatedParams, {
            preserveState: true,
            preserveScroll: true,
        });
    };
                        ...
                        <div className="p-5 text-gray-900 dark:text-gray-100 dark:bg-indigo-950">
                            <div className="overflow-auto w-full">
                                <h1 className="uppercase text-center font-bold text-2xl pb-2">
                                    Data Asuhan Keperawatan {header} {totalCount} Pasien
                                </h1>
                                        ...
                                        <tr>
                                            <th colSpan={8} className="px-3 py-2">
                                                <div className="flex items-center space-x-2">
                                                    ...
                                                    />
                                                    <ButtonTime href={route("askep.filterByTime", "hariIni")} text="Hari Ini" />
                                                    <ButtonTime href={route("askep.filterByTime", "mingguIni")} text="Minggu Ini" />
                                                    <ButtonTime href={route("askep.filterByTime", "bulanIni")} text="Bulan Ini" />
                                                    <ButtonTime href={route("askep.filterByTime", "tahunIni")} text="Tahun Ini" />
                                                </div>
                                            </th>
                                        </tr>
                                    </TableHeader>
                                        ...
                                        {dataTable.data.length > 0 ? (
                                            dataTable.data.map((data, index) => (
                                                <TableRow key={data.id} isEven={index % 2 === 0}>
                                                    ...
                                                    <TableCellMenu>
                                                        <ButtonDetail
                                                            href={route("askep.detail", { id: data.id })}
                                                        />
                                                    </TableCellMenu>
                                                </TableRow>
                                            ))
