                        ...
                        <div className="p-5 text-gray-900 dark:text-gray-100 dark:bg-indigo-950">
                            <div className="overflow-auto w-full">
                                <div className="relative flex items-center justify-between pb-2">
                                    <ButtonBack href={route("pengajuanSep.index")} />
                                    <h1 className="absolute left-1/2 transform -translate-x-1/2 uppercase font-bold text-2xl">DATA DETAIL PENGAJUAN SEP</h1>
                                </div>
                                                ...
                                                <TableHeader>
                                                    <tr>
                                                        {headers.map((header, index) => (
                                                            <TableHeaderCell key={index} className={`${index === 0 ? "w-[5%]" : index === 1 ? "w-[25%]" : "w-[auto]"} 
                                                            ${header.className || ""}`}
                                                            >
                                                                {
                                                                    header.name
                                                    ...
                                                    {tableData.map((detailItem, index) => (
                                                        <TableRow key={index}>
                                                            ...
                                                            <TableCell className="text-wrap">
                                                                {detailItem.uraian === "jnsPelayanan" ? (
                                                                    detailItem.value == 1 ? "Rawat Inap" :
                                                                        detailItem.value == 2 ? "Rawat Jalan" :
                                                                            detailItem.value
                                                                ) : detailItem.uraian === "jnsPengajuan" ? (
                                                                    detailItem.value === 1 ? "Backdate" :
                                                                        detailItem.value === 2 ? "Finger Print" :
                                                                            detailItem.value
                                                                ) : detailItem.uraian === "status" ? (
                                                                    detailItem.value === 1 ? "Pengajuan" :
                                                                        detailItem.value === 2 ? "Approve" :
                                                                            detailItem.value
                                                                ) : detailItem.value}
                                                            </TableCell>
                                                        </TableRow>
