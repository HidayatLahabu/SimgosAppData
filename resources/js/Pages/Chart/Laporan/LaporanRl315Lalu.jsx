...
const LaporanRl315Lalu = ({ tahunLalu, laporanRl315Lalu }) => {
    ...
    useEffect(() => {
        ...
        if (!laporanRl315Lalu || laporanRl315Lalu.length === 0) return;

        // Filter data dengan total pasien lebih dari 100
        const filteredData = laporanRl315Lalu.filter((item) => {
        ...
        const datasets = [
            {
                ...
                backgroundColor: "rgba(173, 255, 47, 0.5)", // Neon green (Chartreuse)
            },
            {
                label: "Rawat Inap",
                data: totalPatientsPerItem.map((item) => item.JMLRI),
                backgroundColor: "rgba(255, 105, 180, 0.5)", // Hot pink
            },
            {
                label: "Laboratorium",
                data: totalPatientsPerItem.map((item) => item.LAB),
                backgroundColor: "rgba(0, 255, 255, 0.5)", // Aqua / Cyan
            },
            {
                label: "Radiologi",
                data: totalPatientsPerItem.map((item) => item.RAD),
                backgroundColor: "rgba(218, 112, 214, 0.5)", // Orchid / Purple pink
            },
        ];
        ...
        <div className="p-5 w-full">
            <div className="bg-white dark:bg-indigo-950 overflow-hidden shadow-sm sm:rounded-lg w-full">
                <div className="p-5 text-gray-900 dark:text-gray-100">
                    <h1 className="uppercase text-center font-bold text-xl">Laporan RL 3.15 - Cara Bayar {tahunLalu}</h1>
...
export default LaporanRl315Lalu;
