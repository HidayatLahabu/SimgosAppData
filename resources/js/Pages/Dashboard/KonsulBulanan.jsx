...
export default function MonthlyKonsulTable({ konsulBulanan = [] }) {
    const [items, setItems] = useState(konsulBulanan);

    useEffect(() => {
        // If data is passed as a prop, use it directly
        if (Array.isArray(konsulBulanan)) {
            setItems(konsulBulanan);
        } else {
            console.error('Invalid data passed to component:', konsulBulanan);
        }
    }, [konsulBulanan]);  // This will run when the prop `konsulBulanan` changes

    // Ensure items is always an array
    const sortedItems = Array.isArray(items) ? [...items] : [];

    return (
        <div className="max-w-full mx-auto sm:px-1 lg:px-1 w-full">
