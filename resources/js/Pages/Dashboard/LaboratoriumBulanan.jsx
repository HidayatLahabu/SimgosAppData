...
export default function LaboratoriumBulanan({ laboratoriumBulanan = [] }) {
    const [items, setItems] = useState(laboratoriumBulanan);

    useEffect(() => {
        // If data is passed as a prop, use it directly
        if (Array.isArray(laboratoriumBulanan)) {
            setItems(laboratoriumBulanan);
        } else {
            console.error('Invalid data passed to component:', laboratoriumBulanan);
        }
    }, [laboratoriumBulanan]); // This will run when the prop `pendaftaranBulanan` changes
