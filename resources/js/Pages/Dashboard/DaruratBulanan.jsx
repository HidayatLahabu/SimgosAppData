...
export default function DaruratBulanan({ rawatDaruratBulanan = [] }) {
    const [items, setItems] = useState(rawatDaruratBulanan);

    useEffect(() => {
        // If data is passed as a prop, use it directly
        if (Array.isArray(rawatDaruratBulanan)) {
            setItems(rawatDaruratBulanan);
        } else {
            console.error('Invalid data passed to component:', rawatDaruratBulanan);
        }
    }, [rawatDaruratBulanan]); // This will run when the prop `pendaftaranBulanan` changes
