...
class DashboardController extends Controller
{
    ...
    //     // Query SQL untuk menghitung statistik
    //     $sql = "
    //         SELECT 
    //             ROUND((SUM(HP) * 100) / (SUM(TTIDUR) * SUM(JMLHARI)), 2) AS BOR,
    //             ROUND(SUM(LD) / SUM(JMLKLR), 2) AS AVLOS,
    //             (SUM(JMLKLR) / SUM(TTIDUR)) AS BTO,
    //             ROUND(((SUM(TTIDUR) * SUM(JMLHARI)) - SUM(HP)) / SUM(JMLKLR), 2) AS TOI,
    //             ((SUM(LEBIH48JAM) * 1000) / SUM(JMLKLR)) AS NDR,
    //             (((SUM(KURANG48JAM) + SUM(LEBIH48JAM)) * 1000) / SUM(JMLKLR)) AS GDR
    //         FROM (
    //             SELECT 
    //                 irs.AWAL, 0 AS MASUK, 0 AS PINDAHAN, 0 AS KURANG48JAM, 
    ...
    //                 MAX(LASTUPDATED) AS LASTUPDATED
    //             FROM informasi.indikator_rs irs
    //             WHERE irs.TANGGAL BETWEEN :tgl_awal AND :tgl_akhir
    //             UNION
    //             SELECT 
    //                 0 AS AWAL, 0 AS MASUK, 0 AS PINDAHAN, 0 AS KURANG48JAM, 
    //                 0 AS LEBIH48JAM, 0 AS DIPINDAHKAN, 0 AS JMLKLR, 0 AS LD, 
    //                 0 AS HP, TTIDUR, 0 AS JMLHARI, irs.SISA AS PASIENDIRAWAT,
    //                 MAX(LASTUPDATED) AS LASTUPDATED
    //             FROM informasi.indikator_rs irs
    //             WHERE irs.TANGGAL = :tgl_akhir
    //         ) AS b;
    //     ";
    ...
    //         return [
    //             'BOR' => is_numeric($result->BOR) ? (float) $result->BOR : 0,
    //             'AVLOS' => is_numeric($result->AVLOS) ? (float) $result->AVLOS : 0,
    //             'BTO' => is_numeric($result->BTO) ? (float) $result->BTO : 0,
    //             'TOI' => is_numeric($result->TOI) ? (float) $result->TOI : 0,
    //             'NDR' => is_numeric($result->NDR) ? (float) $result->NDR : 0,
    //             'GDR' => is_numeric($result->GDR) ? (float) $result->GDR : 0,
    //         ];
    //     }
    ...
    //             0 AS AWAL, 0 AS MASUK, 0 AS PINDAHAN, 0 AS KURANG48JAM, 
    //             0 AS LEBIH48JAM, 0 AS DIPINDAHKAN, 0 AS JMLKLR, 0 AS LD, 
    //             0 AS HP, SUM(TTIDUR) AS TTIDUR, 0 AS JMLHARI, SUM(irs.SISA) AS PASIENDIRAWAT,
    //             MAX(LASTUPDATED) AS LASTUPDATED
    //         FROM informasi.indikator_rs irs
    //         WHERE irs.TANGGAL = '%s'
    //         GROUP BY irs.TANGGAL
    ...
    public function getStatistic()
    {
        ...
        $tgl_awal = Carbon::now()->startOfYear()->toDateString();
        $tgl_akhir = Carbon::now()->toDateString();
        $ttidur = 246; // Definisikan nilai statis untuk TTIDUR
